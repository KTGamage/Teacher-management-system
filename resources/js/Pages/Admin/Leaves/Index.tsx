...
// Friendly status labels and colors
...
export default function AdminLeaves({ leaves }: Props) {
  ...
  const confirmActionHandler = () => {
    ...
    if (confirmAction.type === 'delete') {
      destroy(`/admin/leaves/${confirmAction.leaveId}`, {
        onSuccess: () => {
    ...
    const base = `/admin/leaves/${confirmAction.leaveId}`;
    const url = confirmAction.type === 'approve' ? `${base}/approve` : `${base}/reject`;
  ...
  return (
    <AuthenticatedLayout breadcrumbs={[{ label: 'Leaves' }]}>
      <Head title="Leave Requests" />
      <PageHeader
        title="Leave Requests"
        subtitle="Manage all teacher leave requests."
      />
      ...
      <div className="panel overflow-hidden">
        {leaves.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="mt-4 text-lg font-semibold text-slate-700">No leave requests</h3>
            <p className="mt-1 text-sm text-slate-500">When teachers submit leave requests, they will appear here.</p>
          </div>
        ) : (
                    ...
                    <tr key={leave.id} className="transition hover:bg-slate-50">
                      ...
                      <td className="whitespace-nowrap px-4 py-2 text-sm">
                        <div className="flex items-center gap-2">
                          ...
                          (leave.status === 'pending' && leave.teacher.is_section_head) ? (
                            <>
                              <button
